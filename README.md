...
3. 手机上打开 Shizuku，连接电脑 ⚠️ ️USB 模式需要选择 MTP 模式，如果在仅充电模式下连接过请重启后再激活 Shizuku，不然断开数据线后 Shizuku 服务会被关闭；
...
不出意外就可以正常连接使用了，有需要可以在设置页面调整各种选项， ⚠️ ️系统设置内需要关闭 App 的省电优化
...
- **坚果 R1 / Pro2S（主线）**
  - SmartisanOS 7.2（Android 8.1）
  - 无 Root：不支持 TNT 无头启动，无真实 TNT 显示器时回退手机镜像，音频使用 `audio_loopback`。
  - Root：支持无头启动 TNT，需要启用本项目提供的 Xposed/LSPosed 模块；音频使用 `REMOTE_SUBMIX`。
...
原项目遗留的"安卓屏连App" 有扩展屏相关模块，偏向“外接显示器 / 第二屏 / 输入绑定”能力。
